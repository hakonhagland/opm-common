...
void GroupEconProductionLimits::serializeOp(Serializer& serializer)
{
    serializer(m_groups);
}*/

// DEBUG: Try to realize the specific template version that linker is complaining about here..
template<> void GroupEconProductionLimits::serializeOp<Opm::Serializer<Opm::Serialization::MemPacker>>(
    Opm::Serializer<Opm::Serialization::MemPacker>& serializer);
    ...
    , m_report_step{report_step}
{
    if (record.getItem("END_RUN_FLAG").hasValue(0)) {
        ...
        if (string_endrun == "YES") {
            ...
            throw std::invalid_argument("Unknown input: " + string_endrun + " for END_RUN_FLAG in GECON");
        }
    }
}
