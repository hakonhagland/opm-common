...
public:
    ...
    bool operator!=(const GroupEconProductionLimits& other) const;
    // NOTE: this template function must be fined here in the header (instead of in the .cpp file)
    //  or else the linker will give "undefined reference error" when linking the
    //  test_Serialization binary
    template<class Serializer> void serializeOp(Serializer& serializer)
    {
        serializer(m_groups);
    }
